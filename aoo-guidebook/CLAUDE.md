...
多语言建筑计算器与指南网站。提供免费的混凝土、屋顶、地板、油漆等建筑计算器，以及相关的施工指南文章。支持 18 种语言。
...
│       └── SelectInput.tsx
├── i18n/
│   ├── routing.ts               # 语言配置 (18个语言)
│   ├── navigation.ts            # 导航工具 (Link, redirect 等)
│   ├── request.ts               # 消息加载
│   └── messages/                # 翻译文件 (.json × 18)
├── lib/
│   ├── calculations/
│   │   └── concrete.ts          # 混凝土计算逻辑
│   └── utils/
...
`en, es, fr, de, it, pt, nl, pl, ru, ja, ko, zh, ar, tr, vi, th, sv, da`
...
1. 在对应语言的所有 `messages/*.json` 的 `guides` 命名空间中添加 `mixRatiosGuide` 和 `mixRatiosDesc`
...
- 所有页面通过 `generateStaticParams` 为 18 种语言生成静态页面
