...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: t('deckMistakesGuide'),
    description: t('deckMistakesDesc'),
    keywords: ['DIY deck mistakes','deck building tips','common deck errors','how to build a deck','deck joist spacing','deck footings','deck permits','deck calculator'],
    alternates: getLocalizedAlternates(locale, '/guides/diy-deck-mistakes'),
    openGraph: {
      title: `${t('deckMistakesGuide')} | App Guidebook`,
      description: t('deckMistakesDesc'),
      url: `https://appguidebook.com/${locale}/guides/diy-deck-mistakes`,
      type: 'article',
    },
  };
}
...
const SCENE_ICONS = ['🪵','📋','📏','🔩','🕳️','🌧️','🏆'] as const;
const TAG_COLORS = [
...
export default async function DeckMistakesPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'deckMistakesPage' });
  const tGuides = await getTranslations({ locale, namespace: 'guides' });

  return (
    <div className="py-12">
      <article className="container-custom max-w-4xl">
        <Breadcrumbs items={[{ label: tGuides('title'), href: '/guides' }, { label: tGuides('deckMistakesGuide') }]} locale={locale} />
        <ArticleJsonLd path="/guides/diy-deck-mistakes" locale={locale} headline={tGuides('deckMistakesGuide')} description={tGuides('deckMistakesDesc')} />
        ...
        <header className="mb-12 text-center">
          ...
          <div className="flex items-center justify-center gap-4 mt-6 text-sm text-gray-400"><span>🕐 {t('readTime')}</span><span className="text-gray-200">·</span><span>🏷️ Deck</span></div>
        </header>
        ...
        <SceneCard tag={SCENE_TAGS[1]} icon={SCENE_ICONS[1]} tagColor={TAG_COLORS[1]} bgGradient={BG_GRADIENTS[1]} index={1} cost={t('wasteCost1')}>
          ...
          <div className="grid md:grid-cols-5 gap-4 mt-5">
            ...
            <div className="md:col-span-2 bg-gray-50 border border-gray-200 rounded-xl p-4 text-center flex flex-col justify-center">
              <span className="text-3xl mb-1">🧮</span>
              <div className="text-sm font-bold text-gray-700 mb-2">{t('calculatorName')}</div>
              <span className="text-xs text-gray-500">{t('calculatorDesc')}</span>
            </div>
          </div>
        </SceneCard>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-red-50/70 border border-red-200 rounded-xl p-4 text-center">
                ...
                <div className="font-bold text-red-700 text-sm">{t('wrongFootingTitle')}</div>
                <div className="text-xs text-red-500 mt-1">{t('wrongFootingDesc')}</div>
              </div>
              <div className="bg-green-50/70 border border-green-200 rounded-xl p-4 text-center">
                <div className="text-3xl mb-1">💪</div>
                <div className="font-bold text-green-700 text-sm">{t('rightFootingTitle')}</div>
                <div className="text-xs text-green-600 mt-1">{t('rightFootingDesc')}</div>
              </div>
            </div>
          </div>
        </SceneCard>
        ...
        <section className="mt-16">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Link href="/guides/how-to-build-a-deck" className="card hover:shadow-lg hover:-translate-y-0.5 transition-all block">
              <div className="text-sm font-bold text-primary-600 mb-1">📐 Guide</div>
              <div className="font-semibold text-gray-900">{tGuides('deckBuildGuide')}</div>
              <div className="text-xs text-gray-500 mt-1">{tGuides('deckBuildDesc')}</div>
            </Link>
            <Link href="/guides/deck-materials-calculator" className="card hover:shadow-lg hover:-translate-y-0.5 transition-all block">
              <div className="text-sm font-bold text-primary-600 mb-1">📏 Guide</div>
              <div className="font-semibold text-gray-900">{tGuides('deckMaterialsGuide')}</div>
              <div className="text-xs text-gray-500 mt-1">{tGuides('deckMaterialsDesc')}</div>
            </Link>
            <Link href="/guides/deck-cost-estimation" className="card hover:shadow-lg hover:-translate-y-0.5 transition-all block">
              <div className="text-sm font-bold text-primary-600 mb-1">💰 Guide</div>
              <div className="font-semibold text-gray-900">{tGuides('deckCostGuide')}</div>
              <div className="text-xs text-gray-500 mt-1">{tGuides('deckCostDesc')}</div>
            </Link>
          </div>
        </section>

        <div className="mt-14 border border-gray-200 rounded-2xl p-8 text-center bg-gray-50/50">
          <p className="text-gray-600 mb-5 max-w-xl mx-auto leading-relaxed text-sm">{t('ctaDesc')}</p>
          <Link href="/tools/deck-calculator" className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg border border-gray-300 text-gray-700 text-sm font-medium hover:bg-white hover:border-gray-400 transition-colors">
            {t('tryCalculator')}
