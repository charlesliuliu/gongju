...
import {
  calculateFlooring,
  validateFlooringInput,
  formatArea,
  formatCurrency,
  FlooringResult,
} from '@/lib/calculations/flooring';

export default function FlooringCalculator() {
  const t = useTranslations('flooringCalculator');
  const [length, setLength] = useState<number | ''>(12);
  const [width, setWidth] = useState<number | ''>(12);
  const [wasteFactor, setWasteFactor] = useState<number | ''>(10);
  const [coveragePerBox, setCoveragePerBox] = useState<number | ''>(20);
  const [pricePerBox, setPricePerBox] = useState<number | ''>(40);
  ...
  const handleCalculate = () => {
    const input = {
      length: toNum(length),
      width: toNum(width),
      wasteFactor: toNum(wasteFactor),
      coveragePerBox: toNum(coveragePerBox),
      pricePerBox: showCost ? toNum(pricePerBox) : undefined,
    };

    const validationErrors = validateFlooringInput(input);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="card">
        ...
        <NumberInput
          ...
          required
        />
        <p className="text-xs text-gray-400 -mt-3 mb-4">{t('wasteDesc')}</p>
        ...
        {result ? (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <ResultCard
                title={t('wasteAmount')}
                value={formatArea(result.wasteAmount)}
                unit={t('sqft')}
                icon="📦"
              />
              <ResultCard
            ...
            {!showCost && (
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
                <p className="text-sm text-gray-500">
                  {t('totalArea')}: {formatArea(result.totalArea)} {t('sqft')} &nbsp;|&nbsp;
                  {t('areaWithWaste')}: {formatArea(result.areaWithWaste)} {t('sqft')}
                </p>
              </div>
            )}
