...
export default function DeckCalculator() {
  const t = useTranslations('deckCalculator');
  const [deckLength, setDeckLength] = useState<number | ''>(12);
  const [deckWidth, setDeckWidth] = useState<number | ''>(10);
  const [joistSpacing, setJoistSpacing] = useState<number | ''>(16);
  const [deckingBoardWidth, setDeckingBoardWidth] = useState<number | ''>(5.5);
  const [pricePerSqFt, setPricePerSqFt] = useState<number | ''>(15);
  ...
  const handleCalculate = () => {
    const input = {
      deckLength: toNum(deckLength),
      deckWidth: toNum(deckWidth),
      joistSpacing: toNum(joistSpacing),
      deckingBoardWidth: toNum(deckingBoardWidth),
      pricePerSqFt: showCost ? toNum(pricePerSqFt) : undefined,
    };

    const validationErrors = validateDeckInput(input);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="card">
        ...
        <NumberInput
          ...
          required
        />
        <p className="text-xs text-gray-400 -mt-3 mb-4">{t('joistSpacingDesc')}</p>

        <NumberInput
          label={t('deckingBoardWidth')}
          value={deckingBoardWidth}
          onChange={setDeckingBoardWidth}
          unit={t('inches')}
          min={3.5}
          max={7.25}
          step={0.25}
          required
        />
        <p className="text-xs text-gray-400 -mt-3 mb-4">{t('deckingBoardWidthDesc')}</p>
        ...
        {result ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <ResultCard
                title={t('deckingBoards')}
                value={result.deckingBoards}
                unit={t('boards')}
                icon="🪵"
              />
              <ResultCard
                title={t('numberOfJoists')}
                value={result.numberOfJoists}
                unit={t('pieces')}
                icon="🔩"
              />
              <ResultCard
                title={t('numberOfBeams')}
                value={result.numberOfBeams}
                unit={t('pieces')}
                icon="🏗️"
              />
              <ResultCard
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
              <p className="text-sm text-gray-500">
                {t('totalArea')}: {result.totalArea} {t('sqft')} &nbsp;|&nbsp;
                {t('deckingBoards')}: {result.deckingBoards} &nbsp;|&nbsp;
                {t('numberOfJoists')}: {result.numberOfJoists}
              </p>
            </div>
