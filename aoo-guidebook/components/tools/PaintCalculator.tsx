...
import {
  ...
  formatCurrency,
  PaintResult,
} from '@/lib/calculations/paint';

export default function PaintCalculator() {
  const t = useTranslations('paintCalculator');
  ...
  const [coverage, setCoverage] = useState<number | ''>(350);
  const [pricePerGallon, setPricePerGallon] = useState<number | ''>(40);
  ...
  const handleCalculate = () => {
    const input = {
      ...
      doors: toNum(doors),
      coats: toNum(coats),
      coveragePerGallon: toNum(coverage),
      pricePerGallon: showCost ? toNum(pricePerGallon) : undefined,
    };

    const validationErrors = validatePaintInput(input);
    ...
    setResult(calculatePaint(input));
    setErrors([]);
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="card">
        ...
        <NumberInput
          ...
          required
        />
        <p className="text-xs text-gray-400 -mt-3 mb-4">{t('coatsDesc')}</p>
        ...
        <NumberInput
          ...
          required
        />
        <p className="text-xs text-gray-400 -mt-3 mb-4">{t('coverageDesc')}</p>
        ...
        {result ? (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <ResultCard
                title={t('wallArea')}
                value={formatPaintArea(result.wallArea)}
                unit={t('sqft')}
                highlight
                icon="📐"
              />
              <ResultCard
                title={t('netArea')}
                value={formatPaintArea(result.netArea)}
                unit={t('sqft')}
                highlight
                icon="🎯"
              />
              <ResultCard
                title={t('excludeWindows')}
                value={formatPaintArea(result.windowArea)}
                unit={t('sqft')}
                icon="🪟"
              />
              <ResultCard
                title={t('excludeDoors')}
                value={formatPaintArea(result.doorArea)}
                unit={t('sqft')}
                icon="🚪"
              />
              <ResultCard
                title={t('gallonsNeeded')}
